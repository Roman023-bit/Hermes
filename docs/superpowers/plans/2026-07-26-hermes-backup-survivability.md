...
Таймер KF — ежемесячный по замыслу (`OnCalendar=monthly`,
`RandomizedDelaySec=2h`), поэтому следующий запуск попадает в случайную точку
двухчасового окна в начале месяца. Это не расписание Hermes и менять его здесь
не нужно.
