...
- off-site копия хранится на Mac и обновляется LaunchAgent
  `com.knowledge-factory.backup-pull`; самый новый `daily-*` всегда
  скачивается раньше исторических и protected-архивов;
...
Off-site pull сначала определяет самый новый `daily-*`, полностью скачивает и
проверяет его, и только затем возобновляет старые частичные загрузки. Это
важно при медленном канале Aeza → Mac: исторический архив не должен задержать
появление последней пригодной к восстановлению копии.


## Выживаемость: фактический итог 2026-07-27

Ниже — результат живой выкатки, а не проект. Числа в этом разделе — снимок
состояния на 2026-07-27, а не постоянный норматив: дерево меняется, и
счётчики вместе с ним.

### Knowledge Factory

Восстановительная проверка больше не исполняет содержимое файла из каталога
бэкапов. Строка `source "$latest/STATE"` заменена строгим парсером
`scripts/state_parser.py`: он валидирует **весь** файл, отвергая неизвестные
и повторяющиеся ключи, нецелые значения, malformed-строки и отсутствие любого
из трёх обязательных ключей. Раньше запись в каталог бэкапов давала
выполнение кода от root по ежемесячному таймеру.

Проверено на сервере ручным прогоном:

```
restore_drill_OK documents=96 chunks=621 points=621
  backup=/srv/knowledge-factory/data/backups/daily-20260726T094823Z
```

Таймер возвращён в расписание только после этого прогона. Он **ежемесячный**
по замыслу (`OnCalendar=monthly`, `RandomizedDelaySec=2h`) — еженедельным
является drill Hermes на Mac.

Стягивание бэкапов KF на Mac теперь отказывается работать без активного
FileVault и берёт **общий с Hermes** файловый лок
`~/Library/Application Support/offsite-sync/network.lock` через
`scripts/offsite_lock.py`: канал узкий, и два стягивания не должны делить его.
Дескриптор передаётся дочернему процессу, поэтому убийство обёртки не
освобождает канал, пока идёт перенос.

### Hermes

Essential-бэкап на Aeza, ручной прогон 2026-07-27:

```
hermes_essential_backup_OK path=…/daily-20260727T003341Z
  files=416 unclassified=0 specials=0 escaping_links=18
```

`unclassified=0` означает, что каждый объект, **попавший в staging**, назван
правилом классификации. Исключённое до staging там не учитывается — оно
отдельно перечислено в `EXCLUSIONS.jsonl` вместе с причиной. Ненулевое значение
впредь читается как «появилось что-то новое», а не «правила отстали».

Off-site drill на Mac по скачанной копии:

```
hermes_restore_drill_OK sessions=2 skills=79 plugins=3 cron_jobs=3 unclassified=0
```

Первый заход drill'а закончился отказом — `permissions_too_wide config.yaml 640`.
Права сузили на сервере до `0600` у `config.yaml` и четырёх его исторических
копий, а не ослабили проверку. Сейчас под контролем 10 секретов
(`auth.json`, `sessions/sessions.json`, `config.yaml` и `config.yaml.*`,
`.env` и `.env.*`) — все `0600` и все обычные файлы: симлинк, битая ссылка или
каталог на месте секрета отвергаются. Отдельно проверяется хранилище токенов
`mcp-tokens/`: каталоги внутри обязаны быть ровно `0700`, обычные файлы — не
шире `0600`, и каждый `*.json` должен разбираться. Сейчас там один каталог
`0700` и три файла `0600`.

### Расписания

| Что | Где | Когда | Механизм |
|---|---|---|---|
| Essential-бэкап Hermes | Aeza | ежедневно 03:15 UTC | `hermes-essential-backup.timer` |
| Полный архив Hermes | Aeza | ежедневно 04:15 UTC | `hermes-full-backup.timer` |
| Бэкап Knowledge Factory | Aeza | ежедневно ~02:18 UTC | `knowledge-factory-backup.timer` |
| Drill Knowledge Factory | Aeza | ежемесячно | `knowledge-factory-restore-drill.timer` |
| Стягивание KF | Mac | ежедневно 04:30 | `com.knowledge-factory.backup-pull` |
| Стягивание Hermes + свежесть | Mac | ежедневно 06:00 | `com.hermes.offsite-pull` |
| Drill Hermes | Mac | воскресенье 11:00 | `com.hermes.restore-drill` |

Старая cron-запись root'а в 03:15, вызывавшая `deploy/beget/backup.sh`, снята:
